...
struct Line : Codable {
  let from: CGPoint
  let to: CGPoint
  var objType:String? = nil
}

/// Line connection
struct LineConnection: Codable {
  //let color: UIColor
  let lines: [Line]
}
    ...
    for faceOverlay in faceOverlays {
      drawDots(faceOverlay.dots)
        for lineConnection in faceOverlay.lineConnections {
            if(isBlackFace) {
                drawLinesForBlackFace(lineConnection.lines, lineColor: UIColor.yellow)
            } else {
                drawLines(lineConnection.lines, lineColor: UIColor.red)
            }
      }
    }
  }
